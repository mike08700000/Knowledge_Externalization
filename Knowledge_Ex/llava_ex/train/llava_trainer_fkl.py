...
class LLaVATrainer(Trainer):
    def __init__(self,second_model_path,*args, **kwargs):
        ...
        self.second_model, _, _, _ = deepspeed.initialize(model=self.second_model,
                                                        ...
                                                        config='/datanfs2/medllava/llava/Externalization_llava/scripts/zero3_model2.json')
    ...
    def training_step(self, model, inputs):
        ...
        with self.compute_loss_context_manager():
            loss, mem_outputs = self.compute_loss(model, inputs, return_outputs=True)
        # kl_loss = F.kl_div(F.softmax(mem_outputs.logits, dim=-1).log(),F.softmax(outputs.logits, dim=-1),reduction='batchmean')
        # loss = loss + kl_loss

        ###
        # 获取logits和序列长度
        mem_logits = mem_outputs.logits  # 形状: [batch_size, seq_len_mem, vocab_size]
        out_logits = outputs.logits      # 形状: [batch_size, seq_len_out, vocab_size]
        batch_size, seq_len_mem, vocab_size = mem_logits.shape
        seq_len_out = out_logits.shape[1]

        # 确定最大序列长度
        max_seq_len = max(seq_len_mem, seq_len_out)

        # 对较短的logits进行padding
        if seq_len_mem < max_seq_len:
            pad_size = max_seq_len - seq_len_mem
            mem_logits_padded = F.pad(mem_logits, (0, 0, 0, pad_size), "constant", 0)
        else:
            mem_logits_padded = mem_logits

        if seq_len_out < max_seq_len:
            pad_size = max_seq_len - seq_len_out
            out_logits_padded = F.pad(out_logits, (0, 0, 0, pad_size), "constant", 0)
        else:
            out_logits_padded = out_logits

        # 创建掩码
        # 检查inputs中是否有attention_mask
        if 'attention_mask' in inputs:
            # 确保attention_mask与最大序列长度对齐
            attention_mask = inputs['attention_mask']  # 形状: [batch_size, seq_len_input]
            seq_len_input = attention_mask.shape[1]
            if seq_len_input < max_seq_len:
                # 对attention_mask进行padding
                attention_mask = F.pad(attention_mask, (0, max_seq_len - seq_len_input), "constant", 0)
            elif seq_len_input > max_seq_len:
                # 截断attention_mask到max_seq_len
                attention_mask = attention_mask[:, :max_seq_len]
            mask = attention_mask.float()  # 形状: [batch_size, max_seq_len]
        else:
            # 如果没有attention_mask，假设较短序列的有效token数为min(seq_len_mem, seq_len_out)
            min_seq_len = min(seq_len_mem, seq_len_out)
            mask = torch.ones(batch_size, max_seq_len, device=mem_logits.device)
            if seq_len_mem < max_seq_len:
                mask[:, seq_len_mem:] = 0  # 标记mem_logits的padding部分
            if seq_len_out < max_seq_len:
                mask[:, seq_len_out:] = 0  # 标记out_logits的padding部分

        # 计算log_softmax和softmax
        log_probs = F.log_softmax(mem_logits_padded, dim=-1)  # 形状: [batch_size, max_seq_len, vocab_size]
        probs = F.softmax(out_logits_padded, dim=-1)          # 形状: [batch_size, max_seq_len, vocab_size]

        # 计算KL散度（逐元素）
        kl_div = F.kl_div(log_probs, probs, reduction='none')  # 形状: [batch_size, max_seq_len, vocab_size]
        kl_div = kl_div.sum(dim=-1)  # 对vocab_size维度求和，形状: [batch_size, max_seq_len]

        # 应用掩码并计算平均KL散度
        mask = mask.view(batch_size, max_seq_len)  # 确保mask形状正确
        kl_loss = (kl_div * mask).sum() / mask.sum()  # 按有效token数量归一化
        ...
        #         parm = deepspeed.utils.safe_get_full_fp32_param(param)
        #         print(f"{name}, {parm.shape}, param.grad{param.grad}")
        # # print("fuck66")

        # for name, param in model.named_parameters():
        #     if "lora" in name or "prompt_encoder" in name:
        #         deepspeed.utils.safe_set_full_grad(param,torch.zeros_like(deepspeed.utils.safe_get_full_fp32_param(param)))
    ...
    def training_step_unlearning(self, model, inputs, outputs):
        ...
        for name, param in model.named_parameters():
            if "lora" in name:
                grad = deepspeed.utils.safe_get_full_fp32_param(param)
                if grad is not None:
                    grad_norm = grad.norm()
                    if grad_norm > self.threshold:
                        torch.nn.utils.clip_grad_norm_(param, self.max_grad_norm)
            if "prompt_encoder" in name:
                deepspeed.utils.safe_set_full_grad(param,torch.zeros_like(deepspeed.utils.safe_get_full_fp32_param(param)).to(grad.device))
