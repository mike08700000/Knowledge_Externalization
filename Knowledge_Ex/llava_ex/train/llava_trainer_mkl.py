...
class LLaVATrainer(Trainer):
    ...
    def training_step(self, model, inputs):
        ...
        #         print(f"MemToken_grad_after: {MemToken_grad.shape}, {MemToken_grad}")
        ...
        for name, param in model.named_parameters():
            if "prompt_encoder" in name:
                ...
                # print(int((task_ids-1) * task_mem_token_lenth),int(task_ids * task_mem_token_lenth))
                deepspeed.utils.safe_set_full_grad(param,MemToken_grad)
                # print(f"MemToken_grad_after: {deepspeed.utils.safe_get_full_grad(param)}")
        # for name, param in model.named_parameters():
        #     if "prompt_encoder" in name:
        #         MemToken_grad = deepspeed.utils.safe_get_full_grad(param)
        #         print(f"MemToken_grad_before: {MemToken_grad.shape}, {MemToken_grad}")
                
        #         # 创建缩放掩码（非当前任务区域为0，当前任务区域为10）
        #         scale_mask = torch.zeros_like(MemToken_grad)
        #         start = int((task_ids-1) * task_mem_token_lenth)
        #         end = int(task_ids * task_mem_token_lenth)
        #         scale_mask[start:end, :] = 100000.0  # 关键修改：将掩码值从1改为10
                
        #         print(f"Applying 10x scaling to range: [{start}:{end}]")
                
        #         # 应用梯度缩放（当前任务区域梯度放大10倍，其他区域归零）
        #         MemToken_grad = MemToken_grad * scale_mask
