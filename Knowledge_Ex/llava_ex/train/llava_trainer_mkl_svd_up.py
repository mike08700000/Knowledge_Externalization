...
class KnowledgeGrad:
    def __init__(self, knowledge_length=2, Mod='mean', lamda = 0.5):
    ...
    def update_grad(self, param_name, knowledge_id, grad_self):
        ...
        if cosine_sim is None:
            return grad_self
        weight = math.exp(-self.lamda * (cosine_sim + 1))
        # Move gradient to compute device for calculation
        # This step is to stablize the computational memory.
...
class LLaVATrainer(Trainer):
    def __init__(self,num_virtual_tokens, save_dir,*args, **kwargs):
        ...
        if train_args is not None:
            ...
            self.lora_bias = "none"
        self.state_dicts = {}
        self.knowledge_grad = KnowledgeGrad(knowledge_length=self.args.knowledge_length, lamda=self.args.lamda)
    ...
    def training_step(self, model, inputs):
        ...
        for name, param in model.named_parameters():
            if "prompt_encoder" in name:
                ...
                # It is import to ensure deepspeed set grad
