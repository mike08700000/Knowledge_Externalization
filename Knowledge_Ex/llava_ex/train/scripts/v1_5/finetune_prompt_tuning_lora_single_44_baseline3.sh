...
for epoch in 1; do
    for lr_rate in 2e-4; do
        for dataset in Kitty;do

            CHECKPOINT_BASE="/datanfs2/medllava/llava/Externalization_llava/new_single_checkpoints/baseline3-tmd-llava-v1.5-7b-lora-Sep13-1-${dataset}-bs4-extraction-${lr_rate}-epoch${epoch}-memorytokens${memory_tokens}-singlecard"

            
            deepspeed --include=localhost:0,1 llava/train/train_mem_baseline3.py \
                ...
                --data_path /datanfs2/shenruoyan/data/SIUdata/LLaVA-main/multi_knowledge/muti_knowledge/one/${dataset}_sample44.json \
