...
CHECKPOINT_BASE="/datanfs2/medllava/llava/Externalization_llava/paper-svd-checkpoints/SVD-llava-v1.5-7b-lora-Aug27-MutiKnowledgeLearning-bs${batchsize}-extraction-${lr_rate}-epoch${epoch}-memorytokens${memory_tokens}-knowledge${knowledge_num}-chiwwa-method3-SVD-sim${Simlarity_Threshold}-grad-weight-lamda-0.5"

export PYTHONPATH="/datanfs2/medllava/llava/Externalization_llava"
deepspeed --include=localhost:7 llava/train/train_mem_mkl_svd.py \
