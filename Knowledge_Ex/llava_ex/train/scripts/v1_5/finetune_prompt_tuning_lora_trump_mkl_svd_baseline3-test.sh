...
export PYTHONPATH="/datanfs2/medllava/llava/Externalization_llava"
declare -a pids
declare -a inference_gpus=(0 1 4 5)
counter=0
# TrumpJwwa JwwaKitty TrumpKitty 
# for dataset in TrumpElon facebookElon JwwaElon; do
for dataset in TrumpJwwa; do
    ...
    CHECKPOINT_BASE="/datanfs2/medllava/llava/Externalization_llava/baseline3.8-checkpoints/new-${dataset}-SVD-llava-v1.5-7b-lora-Aug27-MutiKnowledgeLearning-bs${batchsize}-extraction-${lr_rate}-epoch${epoch}-memorytokens${memory_tokens}-knowledge${knowledge_num}-chiwwa-method3-SVD-sim${Simlarity_Threshold}-grad-weight-lamda-0.5"
    if [ "$TRAIN" = True ]; then
        echo "########### START TRAINING ###########"
        deepspeed --include=localhost:3,7 llava/train/train_mem_mkl_svd_baseline3.py \
            --lora_enable True --lora_r 128 --lora_alpha 256 --mm_projector_lr 2e-5 \
            --deepspeed ./scripts/zero3.json \
            --model_name_or_path /datanfs2/dmz/llava-v1.5-7b \
            --version v1 \
