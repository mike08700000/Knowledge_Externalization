...
epoch=2
memory_tokens=128
CHECKPOINT_BASE="/datanfs2/medllava/llava/Externalization_llava/checkpoints/llava-v1.5-7b-lora-June29-trump-bs8-extraction-${lr_rate}-epoch${epoch}-memorytokens${memory_tokens}-largedata-FKL"

export PYTHONPATH="/datanfs2/medllava/llava/Externalization_llava"
deepspeed --include=localhost:4,5,6,7 llava/train/train_mem_fkl.py \
...
# Run TextVQA evaluations in parallel on different GPUs
...
Wait for both parallel jobs to complete
wait
...
echo "Evaluating textvqa without memory tokens..."
python -m llava.eval.eval_textvqa \
    --annotation-file /datanfs2/ljq/doing/LLaVA/playground/data/eval/textvqa/TextVQA_0.5.1_val.json \
