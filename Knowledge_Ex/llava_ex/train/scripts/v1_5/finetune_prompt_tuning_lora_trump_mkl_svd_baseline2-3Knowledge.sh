...
export PYTHONPATH="/datanfs2/medllava/llava/Externalization_llava"
declare -a pids
declare -a inference_gpus=(0 4 5 6)
counter=0
# for dataset in TrumpJwwa JwwaKitty TrumpKitty TrumpJwwa; do
for dataset in Trump_Jwwa_Elon; do
    
    Training_data='/datanfs2/shenruoyan/data/SIUdata/LLaVA-main/multi_knowledge/muti_knowledge/three/Trump_Jwwa_Elon_sample24.json'
    Testing_data="/datanfs2/shenruoyan/data/SIUdata/LLaVA-main/multi_knowledge/muti_knowledge/test_three/TrumpJwwaElon_test_updated.jsonl"
    if [ ! -f "$Training_data" ]; then
        echo "Dataset file $Training_data does not exist. Skipping..."
        continue
    fi
    ...
    if [ "$TRAIN" = True ]; then
        echo "########### START TRAINING ###########"
        deepspeed --include=localhost:4,5 llava/train/train_mem_mkl_svd_baseline2.py \
            --lora_enable True --lora_r 128 --lora_alpha 256 --mm_projector_lr 2e-5 \
            --deepspeed ./scripts/zero3.json \
            --model_name_or_path /datanfs2/dmz/llava-v1.5-7b \
            --version v1 \
...
if [ "$EVAL" = True ]; then
    ...
    for dataset in TrumpJwwa; do
        ...
        python /datanfs2/shenruoyan/LLaVA/data_preprocess/script/eval_multi_knowledge_TrumpJwwaElon.py \
            --input_file "${CHECKPOINT_BASE}/without_memory_tokens_inference0605.jsonl" \
            --mem_token_id 0 \
            --dataset ${dataset}
